...
float elapsedtime, timee, timePrev;
int i;
...
void setup() {
  ...
  timee = millis(); //Start counting timee in milliseconds
  ...
  delay(7000); /*Give some delay, 7s, to have timee to connect
...
/////////////////////////////I M U/////////////////////////////////////
    timePrev = timee;  // the previous timee is stored before the actual timee read
    timee = millis();  // actual timee read
    elapsedtime = (timee - timePrev) / 1000; 
  
  /*The tiemStep is the timee that elapsed since the previous loop. 
   * This is the value that we will use in the formulas as "elapsedtimee" 
...
timee divided by that timee. For taht we will use a variable called previous_error.
We substract that value from the actual error and divide all by the elapsed timee. 
...
//Serial.println(String(pwmRight)+" "+String(pwmLeft)+" "+String(Total_angle[1]));
/*Finnaly using the servo function we create the PWM pulses with the calculated
width for each pulse*/
left_prop.writeMicroseconds(pwmLeft);
left_prop2.writeMicroseconds(pwmLeft);
right_prop.writeMicroseconds(pwmRight);
right_prop2.writeMicroseconds(pwmRight);
